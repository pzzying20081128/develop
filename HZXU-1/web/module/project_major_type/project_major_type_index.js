function create_project_major_type_window(moduleId, moduleName) {
	...
	var mainGridModule = new mainGridWindow({
		moduleId : moduleId,
		// list grid
		url : "./list_ProjectMajorType_list.do",
		// grid_column.record
		record : project_major_type_grid_column.record,
		// grid_column.column
		column : project_major_type_grid_column.column,
		tbar : {
			...
			items : [{
				...
				handler : function(bt) {
					project_major_type_create_windows(moduleId, moduleName, {
						grid : mainGridModule
						,
				...
				handler : function(bt) {
					project_major_type_update_windows(moduleId, moduleName, {
						grid : mainGridModule
						,
				...
				handler : function(bt) {
					project_major_type_delete_windows(moduleId, moduleName, {
						grid : mainGridModule
						,
					});
				}
...
//				});
	...
	/**
	 * var detailModule = new create_project_major_type_detail_window(moduleId +
	 * "_detail", "明细", { mainGrid : mainGrid }); var detailGrid =
	 * detailModule.getGrid();
	 * 
	 ...
	 * sales_order_store_out_panel_print })
	 *  , new Ext.Panel({ id : "222", layout : 'fit', region : 'center', margins :
	 * '0 0 0 0',
	 *  // height : "atuo", title : '明细', items : detailGrid
	 *  })
	 *  ] });
	 * 
	 */
	...
	function project_major_type_delete_windows(moduleId, moduleName, params) {
		...
		showMsgYN({
			...
			yes : function(YN) {
				ERPAjaxRequest({
					url : "./simple_ProjectMajorType_remove.do",
					params : {
