...
import cn.zy.apps.tools.units.SystemConfig ;
 ...
 * for it to actually do anything. If you have JAXP available
 * {@link MyPrivatesProxoolJAXPConfigurator} will do this for you.
 * </p>
 * 
 * <p>
 * Properties that you pass on to the delegate driver have to be treated
 * specially. They must be contained within a &lt;driver-properties&gt; element.
 * </p>
 * 
 * <p>
 * See the <a href=
 * "The latest version is available at http://proxool.sourceforge.net/properties.html"
 * target="_new">Proxool properties</a> for documentation on the available
 ...
 * apart from the elements contained within the &lt;proxool&gt; element.
 * <p>
 * <a name="#validation"> <b>Validation</b><br>
 * A couple of additional steps are required if you want your SAX parser to
 * validate your Proxool xml confguration:
 * <ul>
 * <li>
 * Put your proxool configuration elements inside a root
 * <code>proxool-config</code> element. The document must adhere to the <a
 * href="proxool.dtd">Proxool dtd</a>.</li>
 * <li>
 * Add a <code>DOCTYPE</code> entry to your xml with a system id containing the
 ...
 * <code><nobr>&lt;!DOCTYPE proxool-config SYSTEM "jar:file:///C:/Proxool/lib/proxool.jar!/org/logicalcobwebs/proxool/configuration/proxool.dtd"&gt;</nobr></code>
 * </li>
 * <li>
 * Configure your parser to be validating. In the
 * {@link MyPrivatesProxoolJAXPConfigurator} this is done by passing
 * <code>true</code> as the second arghument to any of the
 * <code>configure</code> methods.</li>
 * </ul>
 * </p>
 * <p>
 ...
 */
public class MyPrivatesProxoolXMLConfigurator extends DefaultHandler {

	private List<String> regPools = new ArrayList<String>();
	private static SystemConfig systemConfig = SystemConfig.instances();
	private static final Log LOG = LogFactory.getLog(MyPrivatesProxoolXMLConfigurator.class);
	...
	public void endElement(String uri, String lname, String qname) throws SAXException {
		...
		if (elementName.equals(PROXOOL)) {
			...
			try {
				...
				if (!regPools.contains(poolName)) {
					ProxoolFacade.registerConnectionPool(url.toString(), properties);
					regPools.add(poolName);
				} else {
					ProxoolFacade.updateConnectionPool(url.toString(), properties);
				}
	...
	private void setProxoolProperty(String localName, String value) {
		if (localName.equals(ProxoolConstants.ALIAS)) {
			...
			poolName = systemConfig.configName(value);

		} else if (localName.equals(ProxoolConstants.DRIVER_CLASS)) {
			// driverClass = value;
			driverClass = systemConfig.configName(value);

		} else if (localName.equals(ProxoolConstants.DRIVER_URL)) {
			// driverUrl = value;
			driverUrl = systemConfig.configName(value);
	...
	private void setDriverProperty(Attributes attributes) throws SAXException {
		...
		final String name = systemConfig.configName(attributes.getValue(NAME));
		final String value = systemConfig.configName(attributes.getValue(VALUE));
	...
	public void error(SAXParseException e) throws SAXException {
		...
		// This should cause the parser stop and an exeption be thrown back to
		// the client.
		throw e;
	}
