...
public class MyPrivatesProxoolConnectionProvider extends ProxoolConnectionProvider {

	private static final String PROXOOL_JDBC_STEM = "proxool.";

	private static final Log log = LogFactory.getLog(MyPrivatesProxoolConnectionProvider.class);
	...
	public void configure(Properties props) throws HibernateException {
		...
		proxoolAlias = systemConfig.configName(props.getProperty(Environment.PROXOOL_POOL_ALIAS));
		...
		if ("true".equals(externalConfig)) {
			...
			// to use
			if (!StringHelper.isNotEmpty(proxoolAlias)) {
				String msg = "Cannot configure Proxool Provider to use JAXP without the " + Environment.PROXOOL_POOL_ALIAS
						+ " property set.";
				log.fatal(msg);
				throw new HibernateException(msg);
			}
			...
			try {
				...
				MyPrivatesProxoolJAXPConfigurator.configure(ConfigHelper.getConfigStreamReader(jaxpFile), false);
			...
			throw new HibernateException("this IIpanelProxoolConnectionProvider not support proxool.properties file ! ");
		}
