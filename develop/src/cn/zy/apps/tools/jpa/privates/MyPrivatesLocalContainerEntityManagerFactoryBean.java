 ...
 * <i>container bootstrap contract. This is the most powerful way to set up a
 ...
 * 
 * <p>
 * As with {@link LocalEntityManagerFactoryBean}, configuration settings are
 * usually read in from a <code>META-INF/persistence.xml config file,
 ...
 * 
 * <p>Internally, this FactoryBean parses the persistence.xml file
 ...
 */
public class MyPrivatesLocalContainerEntityManagerFactoryBean extends AbstractEntityManagerFactoryBean implements ResourceLoaderAware,
		LoadTimeWeaverAware {
	 ...
	 * instance) and linking it in here.
	 * <code>persistence.xml location, DataSource
	 ...
	 * <p>NOTE: Only applied if no external PersistenceUnitManager specified.
	 * 
	 * @param persistenceXmlLocation
	 *            a Spring resource String identifying the location of the
	 *            <code>persistence.xml file that this
	 ...
	 * data source configuration in <code>persistence.xml (if any).
	 * <p>NOTE: Only applied if no external PersistenceUnitManager specified.
	 * 
	 ...
	 * <p>
	 * NOTE: As of Spring 2.5, the context's default LoadTimeWeaver (defined as
	 * bean with name "loadTimeWeaver") will be picked up automatically, if
	 * available, removing the need for LoadTimeWeaver configuration on each
	 * affected target bean.</b> Consider using the
	 * <code>context:load-time-weaver XML tag for creating
	...
	protected EntityManagerFactory createNativeEntityManagerFactory() throws PersistenceException {
		...
		if (provider == null) {
			throw new IllegalStateException("Unable to determine persistence provider. " + "Please check configuration of "
					+ getClass().getName() + "; " + "ideally specify the appropriate JpaVendorAdapter class for this provider.");
		}
