...
// {{{ ----------------------store listeners------------------------------------
    ...
    {
        //alert(" 2 wwwwwwwwwwwwwwwwwwww  onBeforeLoad "+options.forceStart+"  options.params.start   "+options.params.start);
    	 //清空
...
//        alert("sssssssssssss");
    ...
    updateShowInfo:function(totalLength){
    	if(typeof(totalLength)!="undefined") this.ds.totalLength= totalLength;
...
Ext.extend(Ext.ux.grid.livegrid.RowSelectionModel, Ext.grid.RowSelectionModel, {
    ...
    isSelected : function(index)
    {
        if (typeof index == "number") {
            var orgInd = index;
            //自己家的
            if(this.grid.store==null) return false;
...
Ext.ux.grid.livegrid.Toolbar = Ext.extend(Ext.Toolbar, {
    ...
    updateInfo : function(rowIndex, visibleRows, totalCount, context)
    {
        ...
        var msg = totalCount == 0  //selectCountMsg
                ? this.emptyMsg
                :   ( this.isShowSelect ?String.format(this.selectCountMsg,this.selectCount ):"")  +" "+String.format(this.displayMsg, rowIndex+1,
