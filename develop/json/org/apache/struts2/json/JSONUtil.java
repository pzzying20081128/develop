...
public class JSONUtil {
    ...
    public static void writeJSONToResponse(SerializationParams serializationParams) throws IOException {
        ...
        if (StringUtils.isNotBlank(serializationParams.getSerializedJSON())) stringBuilder.append(serializationParams.getSerializedJSON()) ;

        if (StringUtils.isNotBlank(serializationParams.getWrapPrefix())) stringBuilder.insert(0, serializationParams.getWrapPrefix()) ;
        else if (serializationParams.isWrapWithComments()) {
            stringBuilder.insert(0, "/* ") ;
            stringBuilder.append(" */") ;
        } else if (serializationParams.isPrefix()) stringBuilder.insert(0, "{}&& ") ;

        if (StringUtils.isNotBlank(serializationParams.getWrapSuffix())) stringBuilder.append(serializationParams.getWrapSuffix()) ;
        ...
        if (serializationParams.getStatusCode() > 0) response.setStatus(serializationParams.getStatusCode()) ;
        else if (serializationParams.getErrorCode() > 0) response.sendError(serializationParams.getErrorCode()) ;
    ...
    public static Set<String> asSet(String commaDelim) {
        if ((commaDelim == null) || (commaDelim.trim().length() == 0)) return null ;
     ...
     * <p/>
     * Visitation is performed in the following order: aClass aClass' interfaces
     * the interface's superclasses (interfaces) aClass' superclass superclass'
     * interfaces superclass' interface's superclasses (interfaces)
     * super-superclass and so on
     * <p/>
     * The Object base class is base excluded. Classes/interfaces are only
     * visited once each
     * 
     * @param aClass
